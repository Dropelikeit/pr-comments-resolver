...
Match against the current branch. If there is exactly one match, use it. Otherwise ask the user. Use AskUserQuestion to present the options.
...
1. Ask: "Azure DevOps organisation name?" (free-text, required). Use AskUserQuestion to present the options.
2. Ask: "Name of the env variable holding your PAT?" — offer `AZURE_DEVOPS_PAT` as the default and let the user pick another name. Use AskUserQuestion to present the options.
