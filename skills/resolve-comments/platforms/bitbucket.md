...
(Exact subcommand surface depends on the installed `acli` version — verify against `acli bitbucket pr --help` at execution time.) Match the result against the current branch; if there are zero or multiple matches, ask the user. Use AskUserQuestion to present the options.
